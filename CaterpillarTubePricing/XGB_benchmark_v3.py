...
def prepare_data():
    ...
    trainX=np.array(pd.io.parsers.read_table('X_train.csv',sep=',',header=False))
    testX=np.array(pd.io.parsers.read_table('X_test.csv',sep=',',header=False))

    train_ind=[i+1 for i in range(trainX.shape[0])]
    test_ind=[i+1 for i in range(testX.shape[0])]
    labels=np.array(pd.io.parsers.read_table('Y_train.csv',sep=',',header=False))
